...
threatmodel "threatcl-drift-action" {
  ...
  threat "Corpus recordings edited to pass the finding-quality gate" {
    description = "The corpus replay step in .github/workflows/ci.yml (THREATCL_DRIFT_CORPUS=replay running go test ./internal/corpus -v) is the only check in CI that speaks to finding quality rather than to whether the code compiles, and it decides its verdict entirely from files a pull request can edit: each case directory under testdata/corpus holds both the recorded review and the expectation that review is asserted against, so the same pull request that changes prompts/drift-ci.md, the severity rules or the context builder can also rewrite the evidence that its change did no harm. Replay calls no model, so the gate measures the recorded review and never the current engine. One of its two fail-open paths in internal/corpus/corpus_test.go is now closed — a case with no recording fails rather than being skipped, so deleting the recordings is a red build rather than a job that exits 0 having measured nothing. The other remains: a recording whose request digest no longer matches the assembled prompt is reported with t.Logf and still passes, so a prompt change that nobody re-records against leaves the gate asserting a review the current engine would no longer produce. Distinct from the replayed-fixture threat above, whose harm is a reader believing a forged review — here nothing is rendered and nothing is posted, and what is lost is CI's assurance that finding quality has not regressed"
    impacts     = ["Integrity"]
    ...
    control "Fail the replay gate closed" {
      ...
      implemented          = false
      implementation_notes = "Half done. The skip is closed: internal/corpus/corpus_test.go now fails when a case has no recording for the configured provider, so deleting recordings turns the gate red instead of quietly emptying it. The argument that had kept it open — that recordings accumulate one paid run at a time, so a newly added case breaks the build until someone pays — was accepted at the price of the gate being defeatable by omission, and closing it makes recording a case part of adding a case. The digest half is still open and still fails open: a stale recording logs with t.Logf and passes. It was left deliberately rather than overlooked, because failing on it makes every prompt, severity-rule or context-builder edit block on a paid re-record of all seven cases, and that recurring cost is a call for the maintainer rather than a detail of this change. Neither half defends against a pull request that re-records deliberately — that is what review of testdata/corpus diffs is for — but both close the case where the gate is defeated by omission rather than by intent"
      risk_reduction       = 40
    }
  }
  ...
  data_flow_diagram_v2 "review pipeline" {
    ...
    trust_zone "Credentialed Actions runner" {
      ...
      # tags that commit and force-moves the v0 tag onto it.
    ...
    # read here — the refs/tags v0 ruleset admits only the release app, and
    ...
    # release tag created on it, and git push origin -f refs/tags/v0. Distinct
